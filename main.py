...
async def reset_weekly():
    ...
    while True:
        ...
        if ch:
            await ch.send("🔔 รีเซ็ตสัปดาห์ใหม่แล้ว")

# ================== MODAL ==================
class CheckinModal(discord.ui.Modal, title="เช็คชื่อ"):
    note = discord.ui.TextInput(label="หมายเหตุ", required=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not allowed and interaction.user.id in checked_in_users:
            await interaction.response.send_message(
                "❌ คุณเช็คชื่อไปแล้ว", ephemeral=True
            )
            return

        await interaction.response.send_message(
            "📸 ส่งรูปภายใน 60 วินาที", ephemeral=True
        )
...
class CheckinView(discord.ui.View):
    ...
    async def checkin(self, interaction: discord.Interaction, _):
        if not attendance_open:
            await interaction.response.send_message("🔴 ปิดรับ", ephemeral=True)
            return
        ...
        if REQUIRED_TEXT not in interaction.user.display_name:
            await interaction.response.send_message(
                f"❌ ต้องมี `{REQUIRED_TEXT}` ในชื่อ", ephemeral=True
            )
            return
...
async def on_ready():
    ...
    bot.loop.create_task(reset_weekly())
